...
# import assign history data (from csv) & filter out flash assignments ----
...
assign_history$Start <- as.POSIXct(assign_history$Start, format = "%m-%d-%Y %H:%M:%S", tz = "UTC") # UTC is default tz for other funcs
...
# compile entire team & analyst passing history ----
...
# get first L1/L2/L3 team assignment per ticket ----
...
# get first analyst responses for teams L1/L2/L3 ----
...
# import incident list. join response data to incident list ----
OnePOS_Incidents_Import <- readxl::read_excel(path = "\\\\cewp1650\\Chris Jabr Reports\\ONOW Exports\\incident.xlsx")
out <- OnePOS_Incidents_Import %>% left_join(first_L1_response) %>% left_join(first_L2_response) %>% left_join(first_L3_response)
...
# rm(list=ls()[str_detect(ls(), "all_history")])
